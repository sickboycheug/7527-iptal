...
(async () => {
  ...
  if (mode === 'vekil') {
    ...
    await page.evaluate(() => { focusVideo('#preview-content'); setVideoCaption('4. Dilekçeyi kontrol edin ve PDF olarak indirin.'); });
      await page.waitForTimeout(6500);
      await page.evaluate(() => { focusVideo('#download-pdf-btn'); setVideoCaption('5. PDF OLARAK İNDİR düğmesine basın.'); });
      await page.waitForTimeout(5500);
      await page.click('#download-pdf-btn');
      await page.evaluate(() => { focusVideo('#send-btn'); setVideoCaption('6. E-POSTA İLE GÖNDER düğmesine basın. Açılan pencerede PDF dosyasını ekleyin.'); });
      await page.waitForTimeout(7000);
      await page.evaluate(() => { setVideoCaption('7. E-postayı gönderin ve sayfadaki gönderim teyidini verin.'); });
      await page.waitForTimeout(6000);
    await page.evaluate(() => { focusVideo('#t-mb'); setVideoCaption('7. Aynı işlemi TBMM Başkanlığı için bir kez tekrarlayın.'); });
    await page.waitForTimeout(4500);
    await page.click('#t-mb');
    await page.waitForTimeout(2500);
    await page.evaluate(() => { focusVideo('#preview-content'); setVideoCaption('TBMM dilekçesini de kontrol edin, PDF indirip e-postaya ekleyin.'); });
    await page.waitForTimeout(5000);
    await page.evaluate(() => { setVideoCaption('8. Sonra seçtiğiniz milletvekillerine aynı gönderim adımlarını uygulayın.'); });
    await page.waitForTimeout(4500);
