...
(async () => {
  ...
  await page.evaluate(() => { document.getElementById('confirm-modal').style.display = 'none'; focusVideo('#t-mb'); setVideoCaption('9. TBMM Başkanlığı için aynı PDF, e-posta ve teyit sırasını uygulayın.'); }); await page.waitForTimeout(6500); await page.click('#t-mb'); await page.waitForTimeout(2500);
  await page.evaluate(() => { focusVideo('#t-mp'); setVideoCaption('10. Sonra milletvekili için farklı akışa geçin.'); }); await page.waitForTimeout(5000); await page.click('#t-mp'); await page.waitForTimeout(2500);
  await page.evaluate(() => { focusVideo('#vekil-radio-list'); setVideoCaption('11. Vatandaş olarak vekili seçip dilekçe bağlantısını paylaşın.'); }); await page.waitForTimeout(6000);
  await page.evaluate(() => { switchTab('vekil'); focusVideo('#mp-search-input'); setVideoCaption('12. Vekil panelinde kendi adınızı arayıp seçin.'); }); await page.waitForTimeout(5000); await page.fill('#mp-search-input', 'Ahmet'); await page.waitForTimeout(4500); await page.selectOption('#mp-select', { index: 0 });
  await page.evaluate(() => { focusVideo('#mp-verification-email'); setVideoCaption('13. Seçtiğiniz vekil için resmî e-posta doğrulaması isteyin.'); }); await page.waitForTimeout(6500);
  await page.evaluate(() => { focusVideo('#mp-verification-code'); setVideoCaption('14. E-postaya gelen tek kullanımlık kodu girip doğrulayın.'); }); await page.waitForTimeout(6500);
  await page.evaluate(() => { focusVideo('#mp-signature-pad'); setVideoCaption('15. Doğrulama sonrası imzanızı atın ve Meclise sunun.'); }); await page.waitForTimeout(6500);
